""" bruteforce_v4 + recursif methode """
import csv
...
def bruteforce_bt(wallet: Wallet) -> Wallet:

    def add_bt(stock, bt, max_benef):
        
        if bt.left is None:
            if stock.price + bt.cost <= wallet.max_invest:
                new_node = Node()
                new_node.stocks.append(stock)
                new_node.cost = stock.price + bt.cost
                new_node.benef = bt.benef + stock.benefit
                bt.left = new_node
                if new_node.benef > max_benef:
                    best_wallet.stocks = bt.stocks
                    max_benef = bt.benef
        else:
            add_bt(stock, bt.left,max_benef)
        if bt.right is None:
            new_node = Node()
            new_node.cost = bt.cost
            new_node.benef = bt.benef
            bt.right = new_node
        else:
            add_bt(stock, bt.right,max_benef)

    best_wallet = Wallet()
    max_benef=0
    bt = Node()
    for stock in wallet.stocks:
        add_bt(stock, bt, max_benef)    
...
if __name__ == "__main__":
    ...
    t1=time()
    best_wallet = bruteforce_bt(all_stocks)
    t2=time()
